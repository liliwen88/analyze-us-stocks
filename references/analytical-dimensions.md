...
| 任务类型 | 必查维度 | 建议维度 | 总维度数 |
|---------|---------|---------|---------|
| **快速扫描** (5-10 min) | 无固定扩展维度 | 空头持仓(3)或期权流(4)，仅在拥挤/异动明显时 | 0-2 |
| **标准报告** (15-30 min) | 同行对比(11), 资本配置(10)或护城河(7，按公司阶段二选一) | 内幕交易(1), 空头持仓(3), 因子暴露(5), 管理层(8) | 2-5 |
| **深度研究** (30+ min) | 按标的材料性选择 6-9 个维度 | 全部 12 个维度仅在用户明确要求或重大决策时执行 | 6-12 |
| **风险审查** | 3,8,9,12 | 1,7 | 4-6 |
| **行业/主题研究** | 7,9,11,12 | 5,6 | 4-6 |
| **财报后评估** | 1,2,5,8,10 | 3,4,11 | 5-8 |
| **技术面交易** | 3,4,6 | 1,5 | 3-5 |

使用原则：扩展维度是为了回答具体决策问题，不是为了填满报告。若某维度不会改变概率、评分或风险提示，跳过并说明未纳入原因。
